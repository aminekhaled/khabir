<template>
    ...
    <section class="courses">
        ...
        <div id="courses-content" class="courses-content d-flex gap-5 flex-wrap">
            <div class="course-preview-card" v-for="course in coursesWithDiscounts" :key="course.id">
                <div v-if="course.is_best_sell" class="best-seller">الافضل مبيعا</div>
                <router-link :to="'/courses/show-content/'+course.id" style="color: #000;"><img :src="'/media/main/'+course.img" alt="card-pic" class="card-pic"></router-link>
                <div class="category"><strong>{{ course.category.name }}</strong></div>
                <div class="title"><router-link :to="'/courses/show-content/'+course.id" style="color: #000;">{{ course.title }}</router-link></div>
                <div class="author">دورة مقدمة من قبل : {{ course.instractor }}</div>
                <div class="description">
                    {{ course.small_description }}
                </div>
                <div class="rating-box">
                    <span class="number d-flex align-items-center gap-1"><i class="fa-regular fa-user"></i>520</span>
                    <span class="rating d-flex align-items-center"><i class="bx bx-like bx-flip-horizontal"></i>145</span>
                </div>
                <div class="price-box">
        ...
        <div class="slider flex">
            <div class="wrapper flex container-fluid">
                <div class="cards flex row container-fluid" id="trainers-cards">
                    <div v-for="instractor in instractors" :key="instractor.id" class="card">
                        <div class="background">
                            <div :style="{ backgroundImage: `url(../media/default/profile_picture/${instractor.user.image})` }" class="trainer-photo">
                                <div class="roll">
                                    {{ instractor.role }}
                                </div>
                            </div>
                        ...
                        <div class="details">
                            <div class="trainer-data">
                                <h1>{{ instractor.user.name }}</h1>
                                <h4>{{ instractor.job }}</h4>
                            </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            async getCourses() {
                axios.get("api/courses/get-courses", {
                    headers: {
            ...
            async getInstractors() {
                // let token = localStorage.token;
                axios.get("api/instractors/get-instractors", {
                    headers: {
                        Authorization: `Bearer ${this.token}`
                    }
                }).then(response=> {
                    if(response.data.status == 'success') {
                        this.instractors = response.data.instractors;
                    }
                })
            },
            addToCart(course_id) {
